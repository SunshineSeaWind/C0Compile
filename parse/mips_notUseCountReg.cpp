...
//void insertIntoVariableTable(int kind, int address, bool isGlobalData) {
...
//	int kind = MidCodeTable[midCodeTablePtr].part_1 == "int" ? INT : CHAR;
//	insertIntoVariableTable(kind, -1, true);
...
//			insertIntoVariableTable(INT);  //part_1的临时变量是运算的结果，故数据类型为INT
//			pushIntoStack("0");
//		}
//		if (part_2.length() > 0 && part_2[0] == '$' && findVarInVariableTable(part_2) == -1) { //不在变量表中，则加入变量表
//			insertIntoVariableTable(INT);  //part_2的临时变量是运算的结果，故数据类型为INT
//			pushIntoStack("0");
//		}
...
//	while (MidCodeTable[midCodeTablePtr].op != "end") {
//		//常量定义
//		if (MidCodeTable[midCodeTablePtr].op == "const") {
//			const2Mips();
//		}
//		//变量定义（非数组）
//		if (MidCodeTable[midCodeTablePtr].op == "int" || MidCodeTable[midCodeTablePtr].op == "char") {
...
//		}
//		//变量定义（数组）
//		if (MidCodeTable[midCodeTablePtr].op == "intArr" || MidCodeTable[midCodeTablePtr].op == "charArr") {
//			intOrCharArray2Mips();
//		}
//		//加减乘除 + - * /
...
//	//打印每个函数结束时候的变量表
//	outFile << "=======================================" << endl;
//	outFile << "name\tkind\taddress" << endl;
//	for (int i = 0; i < variableTableTop; i++)
//	{
//	VariableItem variableitem = variableTable[i];
//	outFile << variableitem.name << "\t" << variableitem.kind << "\t" << variableitem.address << "\t" << endl;
//	}
//	//outFile << "=======================================" << endl;
...
//		insertIntoVariableTable(kind, 4 * (paraNum - i));//因为最开始的实参位于栈的最底部，故应该4 * (paraNum - i)是真正的偏移量
//		midCodeTablePtr++;
//	}
//}
//
...
//void saveSRegisters() {
//	for (int i = 0; i < 8; i++) {
//		outMipsCode << "sw\t$s" << i << "\t" << 4 * i << "($t8)" << endl;
//	}
//}
////恢复$s0-$s7寄存器中的数值
//void loadSRegisters() {
//	for (int i = 0; i < 8; i++) {
//		outMipsCode << "lw\t$s" << i << "\t" << 4 * i << "($t8)" << endl;
//	}
//}